...
\begin{enumerate}[(a)]
  ...
  \item Using a likelihood ratio test, we want to test the null hypothesis
    ...
    \begin{align*}
      ...
      &= {n_{1+} \choose n_{12}} {n_{2+} \choose n_{21}} \frac{\Gamma(n_{+1} + 1)\Gamma(n_{+2} + 1)}{\Gamma(n+2)}\\
...
@
    ...
    $\Pr(\mathbf{n}|H_0)$ and $\Pr(\mathbf{n}|H_1)$. In this case, we have
...
\begin{enumerate}[(a)]
  \item In the $3 \times 3$ case, with $\mathbf{P} = p_{ij}$ unrestricted, we
    ...
    $\Info_{(ij,kl)}^{-1}$ the asymptotic covariance between $\hat{p}_{ij}$ and $\hat{p}_{kl}$.
