...
\begin{enumerate}[(a)]
  \item In the $3 \times 3$ case, with $\mathbf{P} = p_{ij}$ unrestricted, we
    can ``parameterize'' the transition probability matrix as
    \begin{align*}
      ...
      \end{array} \right]
    \end{align*}
    with parameters $p_{ij}$ with $i \in \{1,2,3\}$ and $j \in \{1,2\}$.\\
    ...
    \end{align*}
    With $\theta = \{p_{ij} \vert i \in \{1,2,3\}, j \in \{1,2\}\}$, we have
    for $i \in \{1,2,3\}$,
    \begin{align*}
    ...
    what we expected.
    % TODO: continue with getting covariance from Fisher information matrix...
