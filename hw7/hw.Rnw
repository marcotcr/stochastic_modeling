...
@
\item Starting at multiple starting values, we get the results shown in the
  following table. As we can see, we converge to different estimates of
  ...
  algorithm. By also outputting the loglikelihood of the incomplete data
...
colnames(results) <- c("$\\rho^{(0)}$", "${\\sigma_1^2}^{(0)}$",
                       ...
                       "$\\ell(x)$")
print(xtable(results), sanitize.colnames.function = function(x){x}, include.rownames=F)
@
