...
%right now though.
\begin{align*}
\E\left[a_i|Y, \hat\sigma_1^2, \hat\sigma_2^2, \hat\rho\right] &= \begin{cases}
  x_i  & \text{if } x_i \ne NA \\
  \hat\rho\hat\sigma_1y_i/\hat\sigma_2 & \text{if } x_i  = NA
  \end{cases} \\
\E\left[b_i|Y, \hat\sigma_1^2, \hat\sigma_2^2, \hat\rho\right] &= \begin{cases}
  y_i  & \text{if } y_i \ne NA \\
  \hat\rho\hat\sigma_2x_i/\hat\sigma_1 & \text{if } y_i  = NA
  \end{cases} \\
\E\left[a_i^2|Y, \hat\sigma_1^2, \hat\sigma_2^2, \hat\rho\right] &= \begin{cases}
  x_i^2  & \text{if } x_i \ne NA \\
  \Var(a_i | b_i) + \E[a_i | b_i]^2 = (1 - \hat\rho^2)\hat\sigma_1^2 + \frac{\hat\sigma_1^2}{\hat\sigma_2^2} \hat\rho^2 y_i^2 & \text{if } x_i  = NA
  \end{cases} \\
\E\left[b_i^2|Y, \hat\sigma_1^2, \hat\sigma_2^2, \hat\rho\right] &= \begin{cases}
  y_i^2  & \text{if } y_i \ne NA \\
  \Var(b_i | a_i) + \E[b_i | a_i]^2 = (1 - \hat\rho^2)\hat\sigma_2^2 + \frac{\hat\sigma_2^2}{\hat\sigma_1^2}\hat\rho^2 x_i^2 & \text{if } y_i  = NA
  \end{cases}
\end{align*}
